...
sig {params(x: T.any(Integer, String)).returns(Integer)}
def non_exhaustive_some_cases(x) # error: Expected `Integer` but found `NilClass` for method result type
  case x
  when Integer
    x
  end
...
sig {params(x: T.any(Integer, String)).returns(Integer)}
def non_exhaustive_some_cases_unreachable_when(x) # error: Expected `Integer` but found `NilClass` for method result type
  case x
  when Integer
    x
  when Symbol
